...
\tt{spread} &
\TmpDescriptionLine{Enlarge or spread a gap by a certain factor.} &
...
\subsubsection{\tt{boxrule}}
\label{sec:option-boxrule}

\TmpOption{boxrule}{dimen}: specifies the thickness of the rule around a
...
\bigskip
\begin{center}
\begin{clozebox}[boxrule=1pt,boxwidth=3cm]
boxrule: 1pt
\end{clozebox}
\begin{clozebox}[boxrule=2pt,boxwidth=3cm]
boxrule: 2pt
\end{clozebox}
\begin{clozebox}[boxrule=3pt,boxwidth=3cm]
boxrule: 3pt
\end{clozebox}
\end{center}
\bigskip
...
\newpage
\subsubsection{\tt{minlines}}
\label{sec:option-minlines}

\TmpOption{minlines}{integer}: \tt{minlines} stands for
“\textbf{min}imum \textbf{lines}”. With the option \tt{minlines} you can
...
\tt{minlines}, the option has no effect at all. The option \tt{minlines}
...
Enlarge or spread a gap by a certain factor.
% O.5 beispielsweise vergrößert die Lücke um die Hälfte ihrer ursprünglichen bereite
O.5, for example, enlarges the gap by half of its original size.
