...
\definecolor{TmpYellow}{rgb}{1, 0.75012, 0}
\newmdenv[
  backgroundcolor=TmpYellow!1,
  linecolor=TmpYellow!50
]{TmpExample}
...
\TeX{} is using the \cmd{\clozesetoption} \TmpSecRef{sec:cmd-clozesetoption}.
...
Only the macro \cmd{\clozefix} \TmpSecRef{sec:command-clozefix} takes the
...
\TmpOption{spacing}{number}: This option provides support for setting the
...
\TmpSecRef{sec:command-clozespace}.
...
|distance| \TmpSecRef{sec:option-distance} is not affected by this option,
...
\cmd{\clozefix} \TmpSecRef{sec:command-clozefix}. The default value of the
